...
bool svp_write(
    ...
    for (size_t i = 0; i < offsets_length; i++) {
        size_t range;
        if (add_overflow(offsets[i].out_offset, offsets[i].length, (long unsigned int *)&range)) {
        ...
        if (add_overflow(offsets[i].in_offset, offsets[i].length, (long unsigned int *)&range)) {
            ERROR("Integer overflow");
            return false;
        }
...
bool svp_digest(
    ...
    size_t range;
    if (add_overflow(offset, length, (long unsigned int *)&range)) {
        ERROR("Integer overflow");
        return false;
    }
    ...
    if (add_overflow((unsigned long) svp_buffer->svp_memory, offset, (long unsigned int *)&position)) {
        ERROR("Integer overflow");
        return false;
    }
