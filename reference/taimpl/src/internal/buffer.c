...
sa_status convert_buffer(
    ...
    if (buffer->buffer_type == SA_BUFFER_TYPE_SVP) {
        ...
        size_t memory_range;
        if (add_overflow(buffer->context.svp.offset, bytes_to_process, (long unsigned int **)&memory_range)) {
        ...
        size_t memory_range;
        if (add_overflow(buffer->context.clear.offset, bytes_to_process, (long unsigned int *)&memory_range)) {
            ERROR("Integer overflow");
            return SA_STATUS_INVALID_PARAMETER;
        }
