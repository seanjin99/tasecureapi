...
sa_status client_thread_verify(
    ...
    // Test clear buffer access in different thread
    if (clear_buffer != nullptr) {
        // Write some data to the clear buffer
        std::copy(in_buffer.begin(), in_buffer.end(), clear_buffer->begin());
        status = SA_STATUS_OK;
    } else {
        status = SA_STATUS_OK;
    }
...
namespace {
    TEST(SaClientThreadTest, nominal) {
        ...
        // Use clear memory buffer instead of SVP
        std::vector<uint8_t> clear_buffer(AES_BLOCK_SIZE);
        std::fill(clear_buffer.begin(), clear_buffer.end(), 0xff);
