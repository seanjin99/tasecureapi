        ...
        if (out != NULL) {
            ...
            if (in != NULL) {
                ...
                if (cipher_context->aead) {
                    ...
                    if (status == SA_STATUS_OK) {
                        char tag_hex[MAX_GCM_TAG_LENGTH * 2 + 1];
                        for (size_t dbg_i = 0; dbg_i < end_parameters.tag_length; dbg_i++) {
                            snprintf(tag_hex + (dbg_i * 2), 3, "%02x", cipher_context->tag[dbg_i]);
                        }
                        tag_hex[end_parameters.tag_length * 2] = '\0';
                        INFO("GCM tag (len=%zu): %s", end_parameters.tag_length, tag_hex);
                    }
                } else {
