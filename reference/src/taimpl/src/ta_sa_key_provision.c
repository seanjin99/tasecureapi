...
sa_status ta_sa_key_provision_netflix(
    ...
    /* The code provided here is only for demo purposes and does not provide a complete
     * provisioning implementation.
     * This code just provides an example to get stored keys,
     * it contains decrypted key which is stored_key->key, stored_key->key_length,
     * and other attributes.
     */
    ...
    do {
        ...
        status = soc_kc_unwrap(&stored_key_encryption, encryptionKey, encryptionKeyLen, (void*)parameters);
        if (status != SA_STATUS_OK) {
            ERROR("soc_kc_unwrap failed for encryption key");
            break;
        }

        // Unwrap HMAC key
        status = soc_kc_unwrap(&stored_key_hmac, hmac, hmacLen, (void*)parameters);
        if (status != SA_STATUS_OK) {
            ERROR("soc_kc_unwrap failed for HMAC key");
            break;
        }

        // Unwrap wrapping key
        status = soc_kc_unwrap(&stored_key_wrapping, wrappingKey, wrappingKeyLen, (void*)parameters);
        if (status != SA_STATUS_OK) {
            ERROR("soc_kc_unwrap failed for wrapping key");
            break;
        }
