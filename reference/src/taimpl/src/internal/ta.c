...
static sa_status ta_invoke_key_provision(
    ...
    if (CHECK_TA_PARAM_IN(param_types[2])) {
     ...
     const void*  keyMagnum = params[1].mem_ref;
     const size_t keyMagnumLen = params[1].mem_ref_size;

     sa_status status = SA_STATUS_OK;
     status = ta_sa_key_provision(ta_key_type, keyMagnum,
         keyMagnumLen, parameters, context->client, uuid);
