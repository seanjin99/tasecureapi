...
static sa_status fields_to_rights(
    ...
    if (parameters != NULL) {
        ...
        rights->id[0] = (uint8_t)((parameters_soc->object_id >> 56) & 0xff);
        rights->id[1] = (uint8_t)((parameters_soc->object_id >> 48) & 0xff);
        rights->id[2] = (uint8_t)((parameters_soc->object_id >> 40) & 0xff);
        rights->id[3] = (uint8_t)((parameters_soc->object_id >> 32) & 0xff);
        rights->id[4] = (uint8_t)((parameters_soc->object_id >> 24) & 0xff);
        rights->id[5] = (uint8_t)((parameters_soc->object_id >> 16) & 0xff);
        rights->id[6] = (uint8_t)((parameters_soc->object_id >> 8) & 0xff);
        rights->id[7] = (uint8_t)(parameters_soc->object_id & 0xff);
    }
